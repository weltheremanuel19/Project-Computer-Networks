...
public class Parser
{
	...
	int stopPosition = -1; // -1 inseamna ca nu exista
	int nrBreakPoint = 0; //
	int nrLiniiProgram = -1;
	...
	public void addBreakPoints(String breakPoints){
		this.breakPoints = this.breakPoints + breakPoints;
	}
	public Parser ( ArrayList<Token> tmp, String breakPoints ) {
		...
		try {
			...
			// citim o singura data programul la inceput
			parse();
		} catch (Exception e) {
	...
	public String executeProgram(){
		...
		if (nrLiniiProgram == -1){
			nrLiniiProgram = statements.size();
			//System.out.println("nrl=" + nrLiniiProgram);
		}
		...
		// string de break-point-uri
		String _breakPoints[] = breakPoints.split(",");

		// array de break-point-uri ca intregi
		int __breakPoints[] = new int[1000];

		// "2" ca propozitie -> 2 ca numar
		for (int i = 0; i < _breakPoints.length; i++) {
			try {
				__breakPoints[i] = Integer.parseInt(_breakPoints[i]);
			} catch (Exception e) {
				__breakPoints[i] = 0;
			}
		}

		// doar daca suntem la inceput retinem nr linii program
		if (nrLiniiProgram == 0)
			nrLiniiProgram = statements.size();

		// System.out.println("*** " + pos + " " + statements.size());

		while (pos < tokens.size()) {
			int current = pos;

			// System.out.println("*********");
			// System.out.println("ss " + statements.size());
			// System.out.println("pos " + pos);

			if (current >= nrLiniiProgram) {
				System.out.println("* Sfarsit Program *");
				break;
			}

			//System.out.println("pozitie: " + (pos) + " / " + nrLiniiProgram);

			// daca exista o pozitie unde ne-am oprit
			// o retinem, altfel incepem de la inceput

			for (int i = 0; i < _breakPoints.length; i++) {
				System.out.println(__breakPoints[i] + " ");
			}

			System.out.println("* Programul a pus executia pe pauza la instructiunea: " + stopPosition + " * ");
			System.out.println("* Programul este la punctul de oprire : " + nrBreakPoint + " / " + _breakPoints.length + " * ");
			System.out.println("* Programul este la linia de oprire : " + nrBreakPoint + " / " + __breakPoints[nrBreakPoint] + " * ");


			// daca am gasit un breakpoint
			// retinem numarul liniei
			// si retinem numarul break point-ului
			// System.out.println(nrBreakPoint + " " + _breakPoints.length + " " + pos + " " + __breakPoints[nrBreakPoint]);
			// System.out.println(nrBreakPoint < _breakPoints.length && pos == __breakPoints[nrBreakPoint]);
			if (nrBreakPoint < _breakPoints.length && pos == __breakPoints[nrBreakPoint]) {
				current = pos;
				Statement statement = null;
				statement = statements.get(current);
				statement.execute();
				//return statement.
				pos++;
				stopPosition = pos;
				nrBreakPoint++;
				// System.out.println("Pozitie de stop 1 setata la " + stopPosition);
			}
			else {
				String output = "";
				while (nrBreakPoint < _breakPoints.length &&
						pos < __breakPoints[nrBreakPoint])
				{
					current = pos;
					Statement statement = null;
					statement = statements.get(current);
				...
				if (nrBreakPoint < _breakPoints.length){
					return output;
				}
				...
				// System.out.println("Pozitie de stop 2 setata la " + stopPosition);
				return output;
			}
		}
		return "";
...
// ******************** Statements
	...
	public class Print extends Statement
	{
		...
		{
			// output!
			System.out.println ( "OUTPUT PROGRAM: " + expression.evaluate().toString() );
			executionOutput = "OUTPUT PROGRAM: " + expression.evaluate().toString();
		}
	}
	...
	// current position
	public class EndFunction extends Statement
	{
		public EndFunction() {}
